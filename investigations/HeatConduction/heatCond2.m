% Heat conduction problem simulation, Demonstration of instability 
function heatCond2
    nTemps= 30;
    %  Data for concrete
    ...
    V=V(:,IX); D=D(IX,IX);
    Ds =diag(D);
    dt= 2/abs(Ds(end-1))% Note that we are taking the last but one...!
    tend= 30;
    nsteps = tend/dt;
    tspan= [0,nsteps*dt];
    y0=   zeros(nTemps,1);% initial conditions
    % Select integrator
