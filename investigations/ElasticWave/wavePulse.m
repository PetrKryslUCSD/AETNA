% Elastic 1D wave, Numerical integration
function wavePulse
    npart= 159;
...
%     Show_FFT( [13,130])
    ...
    function Show_FFT(i)
        figure
        x=y(:,i(1));% this is the signal to transform
        NFFT = 2^nextpow2(length(x)); % the number of points in the FFT
        fs =(1/(t(2)-t(1)));% sampling frequency
        f = fs/2*linspace(0,1,NFFT/2);% Well-sampled frequencies
        Y = fft(x,NFFT)/length(x);% FFT
        Y1 =2*abs(Y(1:NFFT/2));% one-sided FFT
        lh =plot(f,Y1); hold on
        set(lh,'Color', 'r','LineWidth', 3,'LineStyle', '-','Marker', '.');
        x=y(:,i(2));% this is the signal to transform
        NFFT = 2^nextpow2(length(x)); % the number of points in the FFT
        fs =(1/(t(2)-t(1)));% sampling frequency
        f = fs/2*linspace(0,1,NFFT/2);% Well-sampled frequencies
        Y = fft(x,NFFT)/length(x);% FFT
        Y1 =2*abs(Y(1:NFFT/2));% one-sided FFT
        lh =plot(f,Y1); hold on
        set(lh,'Color', 'm','LineWidth', 3,'LineStyle', '-','Marker', '.');
        xlabel (' Frequency f [Hz]')
        ylabel ('One-sided amplitude spectrum |Y(f)|');
%         set(gca,'xlim', [0,5])
    end
