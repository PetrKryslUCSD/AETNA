classdef SimPl
    ...
    methods (Static, Access = public) % Demos
        ...
        function demo1()
            ...
            SimPl.draw_springs(ax,x0,conn,PlainSpringWidth,Gray)
            pause(2)
            
            figure('name', 'Only the springs, undeformed, fancy'); ax=gca;
            axes(ax); cla; axis equal; grid on; labels x y; hold on
            SimPl.draw_springs(ax,x0,conn,0.4,Gray,true)
            pause(2)
            
            figure('name', 'Springs and joints, undeformed, plain'); ax=gca;
            axes(ax); cla; axis equal; grid on; labels x y; hold on
            SimPl.draw_springs(ax,x0,conn,PlainSpringWidth,Gray)
            SimPl.draw_joints(ax,x0,PlainJointRadius,Gray)
            pause(2)
            ...
            SimPl.draw_springs(ax,x0,conn,PlainSpringWidth,Gray)
            SimPl.draw_joints(ax,x0,PlainJointRadius,Gray)
            SimPl.draw_forces(ax,x0,AppliedF,'k',ForceScale);
            pause(2)
            ...
            SimPl.draw_joints(ax,x0,PlainJointRadius,Gray)
            SimPl.draw_forces(ax,x0,AppliedF,'k',ForceScale);
            SimPl.draw_joints(ax,x0(4:6,:),2*PlainJointRadius,Gray)
            pause(2)
            ...
            SimPl.draw_springs(ax,x0,conn,PlainSpringWidth,Gray)
            SimPl.draw_joints(ax,x0,PlainJointRadius,Gray)
            SimPl.draw_springs(ax,x,conn,PlainSpringWidth,Red)
            SimPl.draw_joints(ax,x,PlainJointRadius,Red)
            pause(2)
        end
    ...
    methods (Static, Access = public) % Graphics
        ...
        function draw_extents(ax,xrange,yrange)
            % Set the drawing extents in a plot of a spring-damper-mass system.
            %
            % draw_extents(ax,xrange,yrange)
            %
            % Input:
            ...
            %         draw_extents(ax,xrange,yrange);
            %         draw_joints(ax,X,JointRadius,0.7*[1,1,1]);
            %         draw_springs(ax,X+xscale*scale*dX,kconn,SpringWidth,0.7*[1,0,0])
            %         draw_joints(ax,X+xscale*scale*dX,JointRadius,0.7*[1,0,0]);
            %         drawnow;
            %         pause( 0.001 );
            %     end
        ...
        function draw_springs(ax,x,kconn,SpringWidth,Color,FancyRendering)
            % Draw the springs in a spring-damper-mass system.
            %
            % draw_springs(ax,x,kconn,SpringWidth,Color,FancyRendering)
            %
            % Input:
            % ax = axis object
        ...
        function draw_forces(ax,x,AppliedF,Color,ForceScale)
            % Draw the applied forces for a spring-damper-mass system.
            %
            % draw_forces(ax,x,AppliedF,Color,ForceScale)
            %
            % Input:
            % ax = axis object
        ...
        function draw_joints(ax,x,JointRadius,Color,FancyRendering)
            % Draw the joints of a spring-damper-mass system.
            %
            % draw_joints(ax,x,JointRadius,Color)
            %
            % Input:
            % ax = axis object
            % x  = Locations of joints in the undeformed configuration.  Each joint has
        ...
        function draw_joint_numbers(ax,x,Color,FontSize,Offset)
            % Draw the numbers of the entities of a spring-damper-mass system.
            %
            % draw_joint_numbers(ax,x,Color,FontSize,Offset)
            %
            % Input:
            % ax = axis object
            % x  = Locations of joints in the undeformed configuration.  Each joint has
        ...
        function draw_spring_numbers(ax,x,kconn,Color,FontSize,Offset)
            % Draw the numbers of the entities of a spring-damper-mass system.
            %
            % draw_spring_numbers(ax,x,kconn,Color,FontSize,Offset)
            %
            % Input:
            % ax = axis object
            % x  = Locations of joints in the undeformed configuration.  Each joint has
        ...
        function draw_dampers(ax,x,cconn,DamperWidth,Color,FancyRendering)
            % Draw the dampers in a spring-damper-mass system.
            %
            % draw_dampers(ax,x,cconn,DamperWidth,Color,FancyRendering)
            %
            % Input:
            % ax = axis object
            % x  = Locations of joints.  Each joint has
        ...
        function draw_damper_numbers(ax,x,cconn,Color,FontSize,Offset)
            % Draw the numbers of the entities of a spring-damper-mass system.
            %
            % draw_spring_numbers(ax,x,kconn,Color,FontSize,Offset)
            %
