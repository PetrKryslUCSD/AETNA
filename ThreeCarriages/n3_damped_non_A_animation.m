% Standard eigenvalue problem, animation, non-proportionally damped. 
function n3_damped_non_A_animation
    mode =1;

    [M,C,K,A,k1,k2,k3,c1,c2,c3] = properties_damped_non_proportional;
    nparticles =size(M,1);
    ...
    for i =1:length(t)
        ...
        figure(f);
        cla
        render(translate(glyph_rectangle([3,3],'w','w'), [58,34]/2));
        render(Ground);
        ...
        if (k1~=0)
            s =[-8, 0]+[0,-0.8];
            e =p1+[real(phasors(1)), 0]+[-6/2,-0.8];
            render(glyph_spring(s,e,1.9/norm(e-s)*Spring_length));
        end
        if (k2~=0)
            s =p1+[real(phasors(1)), 0]+[6/2,-0.8];
            e =p2+[real(phasors(2)), 0]+[-6/2,-0.8];
            render(glyph_spring(s,e,1.9/norm(e-s)*Spring_length));
        end
        if (k3~=0)
            s =p2+[real(phasors(2)), 0]+[6/2,-0.8];
            e =p3+[real(phasors(3)), 0]+[-6/2,-0.8];
            render(glyph_spring(s,e,1.9/norm(e-s)*Spring_length));
        end
        if (c1~=0)
            s =[-8, 0]+[0, 0.8];
            e =p1+[real(phasors(1)), 0]+[-6/2,0.8];
            render(glyph_damper(s,e,1.9/norm(e-s)*Spring_length));
        end
        if (c2~=0)
            s =p1+[real(phasors(1)), 0]+[6/2,0.8];
            e =p2+[real(phasors(2)), 0]+[-6/2,0.8];
        ...
        g= glyph_vector([0, 0],vscale*[real(phasors(1)),imag(phasors(1))], 2.1);
        g.linewidth =4;
        g.edgecolor ='g';
        render(translate(g,p1+[0, 10]));
        g= glyph_vector([0, 0],vscale*[real(phasors(4)),imag(phasors(4))], 2.1);
        g.linewidth =4;
        g.edgecolor ='r';
        render(translate(g,p1+[0, 10]));
        
        render(translate(Axes,p2+[0, 10]));
        g= glyph_vector([0, 0],vscale*[real(phasors(2)),imag(phasors(2))], 2.1);
        g.linewidth =4;
        g.edgecolor ='g';
        render(translate(g,p2+[0, 10]));
        g= glyph_vector([0, 0],vscale*[real(phasors(5)),imag(phasors(5))], 2.1);
        g.linewidth =4;
        g.edgecolor ='r';
        render(translate(g,p2+[0, 10]));
        
        render(translate(Axes,p3+[0, 10]));
        g= glyph_vector([0, 0],vscale*[real(phasors(3)),imag(phasors(3))], 2.1);
        g.linewidth =4;
        g.edgecolor ='g';
        render(translate(g,p3+[0, 10]));
        g= glyph_vector([0, 0],vscale*[real(phasors(6)),imag(phasors(6))], 2.1);
        g.linewidth =4;
        g.edgecolor ='r';
        render(translate(g,p3+[0, 10]));
