classdef  pun
    ...
    %
    %     m=pun(struct('system_of_units','SI')); %
    %     m('Pa')/m('psi') % What is one Pascal in PSI?
    % 
    %     m=pun(struct('system_of_units','US')); %
    %     E=200*m('giga*Pa')
    %     E/m('psi')
    ...
    methods
        ...
        function  self=pun(Parameters)
            %  Constructor
            % Parameters:
            %    system_of_units='SI';
            %    base_time_units='SEC';
            % See discussion of constructors in <a href="matlab:helpwin 'featbox/classes/Contents'">classes/Contents</a>.
