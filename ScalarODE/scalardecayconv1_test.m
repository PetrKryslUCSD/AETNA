% Convergence of solutions to a scalar equation. W/ odemeul
rhsf =@(t,y) (-0.5*y);
y0=1;
tspan =[0  4];
dts = [1/2, 1/3, 1/10, 1/40];
dts = [1/2, 1/3, 1/10, 1/40, 1/110, 1/150];
...
figure;
loglog(dts(1:end-1),abs(ea_f)./(1-alp.^1), 'linewidth', 2, 'color', 'blue', 'marker', 'o');hold on
loglog(dts(1:end-1),abs(ea_b)./(1-alp.^1), 'linewidth', 2, 'color', 'red', 'marker', 'o');hold on
loglog(dts(1:end-1),abs(ea_m)./(1-alp.^2), 'linewidth', 2, 'color', 'k', 'marker', 'o');hold on
